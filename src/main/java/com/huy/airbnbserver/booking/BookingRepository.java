...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query(value = """
            ...
            FROM booking WHERE property_id = :propertyId AND status NOT IN ('REJECTED','CANCEL')
            UNION
            SELECT DISTINCT DATE(check_out_date) AS booking_date
            FROM booking WHERE property_id = :propertyId AND status NOT IN ('REJECTED','CANCEL')""", nativeQuery = true)
